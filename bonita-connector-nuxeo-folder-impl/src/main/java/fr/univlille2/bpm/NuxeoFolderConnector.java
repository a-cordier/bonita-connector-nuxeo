...
/**
 * @author acordier Jul 24, 2014 NuxeoFolderConnector.java
 */
public class NuxeoFolderConnector extends NuxeoConnector {
	 ...
	 * 
	 * @see
	 * fr.univlille2.bpm.NuxeoConnector#executeConnector(org.nuxeo.ecm.automation
	 * .client.Session)
	 */
	@Override
	protected void executeConnector(final Session session) throws Exception {
		...
		DocumentService documentService = session
				.getAdapter(DocumentService.class);// Nuxeo document API call
		Document folder = null;
		/*
		 * Get or create document
		 */
		try {
			folder = NuxeoConnectorUtils.getDocumentNotDeleted(documentService, path);
		} catch (Exception e) {
			try {
				if (create) {
					folder = documentService.createDocument(parentPath,
							new Document(folderName, "Folder"));
					folder.set("dc:title", folderName);
					documentService.update(folder);
				} else {
					logger.warning(String.format(
							"Folder not found at %s on nuxeo repository", path));
				}
			} catch (Exception nested) {
				logger.warning(String.format(
						"Can not create document at %s, %s", path,
						nested.getMessage()));
			}
		} finally {
			session.close();
		 ...
		 * set permissions
		 */
		for (Object username : NuxeoConnectorUtils.nullToEmpty(usernames)) {
			try {
				logger.info(String.format("Granting permissions %s to %s",
						permissions, username));
				documentService.setPermission(folder, (String) username,
						permissions);
				documentService.update(folder);
			} catch (Exception e) {
				logger.warning(String.format(
						"Can not set permission on document %s, to %s", path,
						username));
